...
<p>Content of second paragraph</p>
  ...
  (** [a ~attrs ~title ~url label] is a link around the text of [label],
      ...
      See {!Link}. *)
  val a : ?attrs:attributes -> ?title:string -> url:string -> string -> attributes inline
  ...
  (** [p ~attrs inlines] is a paragraph block holding the given [inline]
  ...
  (** [html_bl ~attrs blocks] is a block-level element of raw HTML that
      contains nested markdown blocks. See {!Html_block}. *)
  val html_bl
    :  ?attrs:attributes
    -> tag:string
...
module Impl : Intf = struct
  ...
  let ul ?(attrs = []) ?(spacing = Loose) items = List (attrs, Bullet '-', spacing, items)
  ...
  let html_bl ?(attrs = []) ~tag (blocks : attributes block list) =
    Html_block (attrs, tag, blocks)
  ;;
